...
export class MrTreeProvider implements vscode.TreeDataProvider<vscode.TreeItem> {
  ...
  listOpenMrs(): readonly MergeRequestSummary[] {
  ...
  async getChildren(element?: vscode.TreeItem): Promise<vscode.TreeItem[]> {
    if (!element) {
      if (!this.getClient()) {
        const connect = new vscode.TreeItem("Conectar — colar token GitLab");
        connect.iconPath = new vscode.ThemeIcon("key");
        connect.contextValue = "authAction";
        connect.command = { command: "reviewKit.configureToken", title: "Configure GitLab Token" };
        return [connect];
      }
